...
- Linux 与 Windows 均读取完整成绩字段，继续保留 FC / FS / AP / SYNC 等特殊标识。

### v0.6.28

- 修复迁移到 UTC 等非中国时区服务器后，SGID 被误判为晚于当前系统时间的问题。
- SGID 时间校验失败时记录不含凭据内容的时区与时间差诊断日志。

### v0.6.27

- 修复 AstrBot 热更新后旧接口模块残留导致仍走旧链路的问题。
